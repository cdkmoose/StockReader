...
namespace DavinSys.StockReader.Utility
{
	[Serializable()]
	public class SortableBindingList<T> : BindingList<T>, ITypedList
	{
		...
		internal class PropertyComparer<TKey> : System.Collections.Generic.IComparer<TKey>
		{
			...
			* with changes specific to StockReader added by C Pulsifer
			*/
			...
			private bool _isNumericNA;

			public PropertyComparer(PropertyDescriptor property, ListSortDirection direction)
			{
				_property = property;
				_direction = direction;
				_isNumericNA = !(property.Name == "TickerSymbol");
			}
			...
			{
				int result;
				if (_isNumericNA == true)
				{
					result = NumericNACompare(xValue, yValue);
				}
			...
			private int NumericNACompare(object xVal, object yVal)
			{
				double x;
				double y;

				if (xVal.ToString() == yVal.ToString())
				{
					return 0;
				}
				else if (double.TryParse(CleanString(xVal), out x) && double.TryParse(CleanString(yVal), out y))
				{
					return x.CompareTo(y);
				}
				else if (xVal.ToString() == "N/A")
				{
					return -1;
				}
				else
			...
			{
				string result;
				result = obj.ToString().Trim(delim);

				return String.Join("", result.Split(comma));
			}
		}
		#endregion
